...
3. Use `nui-editor handoff|resume|publish` for editor work. Use `nui-screen-extract` and `nui-screen-update` for
   a one-screen change; never replace unrelated screens or metadata. For a component/section, use
   `nui-fragment-extract` and atomic project-mode `nui-fragment-import`: dry-run, apply, repeat the dry-run,
   then verify occurrence geometry/shared defaults. Never hand-splice fragment JSON.
