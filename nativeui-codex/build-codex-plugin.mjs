...
//   plugins/nativeui/lib/
//   plugins/nativeui/mcp/
...
const VERSION = '0.3.0+codex.20260724043824';
const DESCRIPTION =
...
async function build({ outRoot, pluginSrc }) {
  ...
  const libSrc = path.join(pluginSrc, 'lib');
  const mcpSrc = path.join(pluginSrc, 'mcp');
  ...
  await assertDir(libSrc, 'lib source');
  await assertDir(mcpSrc, 'mcp source');
  ...
  await copyDir(libSrc, path.join(pluginOut, 'lib'));
  await copyDir(mcpSrc, path.join(pluginOut, 'mcp'));
