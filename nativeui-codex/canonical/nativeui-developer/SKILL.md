...
3. **Sync guard.** If the project has a cloud name/id or a `.nativeui-sync.json` sidecar, run:
   ```bash
   node ${CLAUDE_SKILL_DIR}/../../bin/nui-project-sync.mjs status project.json --name "Project Name" --human
   ```
...
4. **Register API/database surfaces.** Add or update library items before connector code:
   ```bash
   node ${CLAUDE_SKILL_DIR}/../../bin/nui-library.mjs upsert-api project.json --name login --path /api/login --method POST
   node ${CLAUDE_SKILL_DIR}/../../bin/nui-library.mjs upsert-database project.json --name trips --connector postgresql --table trips
   ```
   Store secrets only through stdin:
   ```bash
   printf '%s' "$SECRET_VALUE" | node ${CLAUDE_SKILL_DIR}/../../bin/nui-library.mjs put-secret \
