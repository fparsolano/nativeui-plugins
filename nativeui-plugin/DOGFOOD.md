...
| 10a | **Atomic fragment dogfood** | Extract one stable-id component, edit its HTML, run `nui-fragment-import --project project.json --replace <id> --dry-run`, apply the same operation, then repeat the dry-run; add `--update-shared-library` for a shared target | Both dry-runs pass, the second is a no-op, the occurrence shell and unrelated shared defaults are unchanged, and no fragment JSON is hand-spliced. |
...
- A clean **prod** export is an idiomatic runnable output without parity instrumentation. Validation proves the
  selected structural/export contracts, not complete visual or OpenJFX behavioral parity; use the applicable
  target tests, runtime acceptance, and fail-closed conformance ledgers.
