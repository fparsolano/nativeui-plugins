...
Repository contributors use the controlled Node.js 24 toolchain. The published
dependency-free plugin itself supports Node.js 18 or newer and keeps explicit
Node 18/20 compatibility coverage. Hosted import, export, editor, and save operations require NativeUI browser sign-in,
...
The plugin ships a dependency-free stdio MCP server at `mcp/server.mjs`. Claude Code registers it through the plugin manifest; Cursor can use `.cursor/mcp.json`; Codex users can copy `admin/codex-mcp.example.toml` into their Codex configuration after resolving the installed plugin path.

When `nativeui_*` MCP tools are available, they are the preferred path for hosted import, export, validation, cloud project, library, fragment, screen, and parity operations. Start with `nativeui_auth_status`. If it returns `authenticated: false`, call `nativeui_login_start`, present its returned browser URL and approval code to the user, then call `nativeui_login_wait`. Every protected MCP operation independently verifies the session; if it is missing or expired, it returns that same complete browser-login handoff and does not begin the requested work. NativeUI presents three distinct concept cards for a user to choose or blend before implementation, then offers opt-in named cloud sync; `nativeui_project_sync` pulls web-editor-only edits, pushes local-only edits, and stops on conflicts. The `bin/` CLIs remain supported for local workflows and secret entry; use `nui-library.mjs put-secret --secret-stdin` for secret values.
