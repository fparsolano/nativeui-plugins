...
node bin/nui-project-sync.mjs sync project.json --name "My App"
# Before and after every later local edit, run the same sync command.
...
# Granular change: public atomic splice, with mandatory before/after dry-runs.
node bin/nui-fragment-extract.mjs project.json --id trip_card -o card.html
node bin/nui-fragment-import.mjs card.html --project project.json --replace trip_card --dry-run
node bin/nui-fragment-import.mjs card.html --project project.json --replace trip_card
node bin/nui-fragment-import.mjs card.html --project project.json --replace trip_card --dry-run
node bin/nui-project-sync.mjs sync project.json --name "My App"
...
`nui-project-sync sync` is the guarded two-way cloud loop: it creates/adopts a named project, pulls cloud-only
web-editor changes, pushes local-only changes, and exits with a conflict instead of overwriting when both changed.
It runs when invoked; it is not a background file watcher.

Every replacement computes a no-op importer baseline and applies only the authored HTML delta, preserving
canonical fields and children the HTML projection cannot represent. For a Library-backed fragment, add
`--recipe <file>` as needed and `--update-shared-library`; that updates only the shared definition while
preserving the thin occurrence shell. The repeated dry-run must be a no-op; also verify occurrence geometry and
unrelated shared defaults before export. Do not manually merge fragment `rootNodes` or `libraryItems`.
