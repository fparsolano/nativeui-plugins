...
export function validateExportManifest(manifest, {
  ...
  if (webTargetIds.length) {
    ...
    for (const id of webTargetIds) {
      ...
      const requiredCommands = id === 'web-html' || id === 'web-shopify'
        ? ['run', 'release']
...
export function webArtifactLayout(root, targetId, renderMode = 'static') {
  if (!['web-html', 'web-react', 'web-vue', 'web-angular', 'web-astro', 'web-shopify'].includes(targetId)) {
  ...
  if ((targetId === 'web-html' || targetId === 'web-shopify') && renderMode !== 'static') {
    throw new ExportManifestError(targetId + ' supports only the static render mode.');
  }

  let clientDir;
  let serverEntry = '';
  if (targetId === 'web-html' || targetId === 'web-shopify') clientDir = '';
