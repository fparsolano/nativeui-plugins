...
// Decode the current canonical InteractionSpec wire shape into the small planner view. The model
// owns the nested identifier wrappers and ActionRuntimePayload; the planner must not depend on the
// retired flat InteractionState compatibility shape.
function normalizeInteractionSpec(spec) {
  const action = spec?.action && typeof spec.action === 'object' ? spec.action : {};
  return {
    id: spec?.id,
    trigger: typeof spec?.triggerId?.value === 'string' ? spec.triggerId.value : '',
    action: typeof action?.id?.value === 'string' ? action.id.value : '',
    targetStageId: action.targetStageId,
    targetNodeId: action.targetNodeId,
    targetLibraryItemId: action.targetLibraryItemId,
    params: action.parameters && typeof action.parameters === 'object' ? action.parameters : {},
  };
}

// Collect every canonical interaction in the project, tagged with where it came from.
function collectInteractions(project) {
  const out = [];
  for (const st of Array.isArray(project.stages) ? project.stages : []) {
    const stageName = typeof st.name === 'string' ? st.name : st.stageId || 'stage';
    for (const spec of Array.isArray(st.interactionSpecs) ? st.interactionSpecs : []) {
      if (spec && typeof spec === 'object') {
        out.push({ ix: normalizeInteractionSpec(spec), where: `stage "${stageName}"`, nodeId: null });
      }
    }
  }
  walkNodes(project, (n) => {
    for (const spec of Array.isArray(n.interactionSpecs) ? n.interactionSpecs : []) {
      if (spec && typeof spec === 'object') {
        out.push({
          ix: normalizeInteractionSpec(spec),
          where: `node "${n.id || n.kind || '?'}"`,
          nodeId: n.id || null,
        });
      }
    }
