...
function explicitPreviewWidths(text) {
  const value = String(text || '');
  const widths = [];
  const dimensionPair = /\b(\d{3,5})\s*(?:x|\u00d7|by)\s*(\d{3,5})\b/gi;
  let match;
  while ((match = dimensionPair.exec(value))) {
    const context = value.slice(Math.max(0, match.index - 56), match.index + match[0].length + 56);
    if (!/\b(?:viewport|screen|window|canvas|preview|device|mobile|tablet|desktop|web|responsive|width|from|to|between)\b/i.test(context)) {
      continue;
    }
  ...
  const widthList = /\b(?:viewport|window|preview|target)\s+widths?\s*(?:are|:|=)?\s*((?:\d{3,5}(?:px)?\s*(?:,|and|to|-)?\s*)+)/gi;
  while ((match = widthList.exec(value))) {
    for (const number of match[1].matchAll(/\d{3,5}/g)) {
      const width = Number(number[0]);
      if (width >= 240 && width <= 10000) widths.push(width);
    }
  }
  return unique(widths).sort((a, b) => a - b);
}
...
function finalize(bundle) {
  ...
  bundle.responsiveTargets = unique([
    ...bundle.responsive.breakpoints,
    ...explicitPreviewWidths(bundle.prompt),
  ]).sort((a, b) => a - b);
  bundle.responsiveTargetPolicy = bundle.responsiveTargets.length
    ? 'explicit-source-or-prompt-snapshots'
    : 'derive-validation-snapshots-from-selected-targets-and-content';
  const kinds = {};
  ...
  bundle.nextSteps = [
    'Use this bundle to plan screens, tokens, components, assets, repeaters, parent constraints, and content-required responsive changes before authoring NativeUI HTML/CSS. When no explicit support bounds exist, derive validation snapshots from the selected delivery targets and content constraints. A validation snapshot is never a preferred authoring canvas.',
