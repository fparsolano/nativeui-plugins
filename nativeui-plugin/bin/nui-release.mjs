...
export function planFor(target, project, opts = {}) {
  ...
  if (id === 'web-shopify') {
    if (renderMode === 'ssr') {
      throw new Error('web-shopify supports only --render-mode static; the storefront renders Liquid.');
    }
    return {
      ...base,
      renderMode: 'static',
      build: [],
      artifacts: ['layout/theme.liquid', 'config/settings_schema.json', 'templates/index.json'],
      deploy: ['shopify', 'theme', 'push'],
      deployment: {
        automated: false,
        supportedProviders: [],
        target: 'Shopify store',
        reason: 'Theme upload requires shopify CLI authentication against a store; run `shopify theme push` from the theme directory.',
      },
    };
  }
