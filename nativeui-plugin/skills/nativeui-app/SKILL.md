...
## 2c. Three concepts — user choice is required
Before planning screens or writing HTML, present **exactly three** concept cards for the user's app idea. Each
must name a distinct visual/product direction and summarize its hierarchy/navigation, responsive layout
character, interaction/motion character, and primary journey. Honor any fixed brand/content constraints in all
three; do not create three palette-only variants. Ask the user to choose one or explicitly blend named elements.
Do not author, import, save, or scaffold a backend until they choose, unless they explicitly ask to skip concepts.
...
Follow `${CLAUDE_SKILL_DIR}/../nativeui/references/authoring-rules.md` exactly. Write each screen as a complete
...
Copy from `${CLAUDE_SKILL_DIR}/../nativeui/examples/` as templates. If `nativeui-design-guide.md` exists, treat
...
### 6b. Offer named cloud sync
After a valid `project.json` exists, ask whether the user wants a cloud project that stays reconciled with the
NativeUI visual web app. If yes, ask for a required project name and optional folder, then run:
```bash
node <bin>/nui-project-sync.mjs sync project.json --name "My App" [--location "Folder"]
```
For every later agent edit, run `sync` before changing local files and again after validation. It pulls cloud-only
web-editor edits, pushes local-only agent edits, and stops on concurrent changes rather than overwriting either
side. This is workflow-driven sync, not a background watcher.
