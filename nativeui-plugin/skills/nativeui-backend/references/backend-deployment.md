...
Every recipe ends the same way: once `GET https://<your-url>/health` is green, flip the **prod** value of the
single base-URL constant in `NuiBackend.{kt,swift}` (§1) to the deployed origin, and drop the dev cleartext/ATS
exceptions from the release build (prod is HTTPS).
