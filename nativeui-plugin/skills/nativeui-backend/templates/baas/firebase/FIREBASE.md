...
server code). The Kotlin/Swift snippets below are **flagship-mobile examples**. When Rust, C#, or an authored
web lane is selected, implement the same contract through that target's manifest-declared action/data seams
instead of creating unselected mobile projects. For the flagship pair, the app talks to Firebase from app-owned
`*BackendConnector.{kt,swift}` classes, with `NuiBackend.{kt,swift}` kept as thin delegators. Two clear lanes:
...
### Flagship-mobile sketches (implement both only when the flagship pair is selected)
```kotlin
...
## 7. Implement every selected target
Keep equivalent `target` cases and result shapes in every selected target's manifest-declared durable seam.
When the flagship mobile pair is selected, implement both Android and iOS connector files and keep
`NuiBackend.kt` and `NuiBackend.swift` as thin delegators. Rust, C#, and web use their own declared seams.
Behavior only — anything visual stays in the HTML/CSS design and is re-imported.
