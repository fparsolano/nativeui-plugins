...
The NativeUI iOS/Android exporters generate the UI + the `NuiScreenControls`/`NuiScreenDelegate`
contract, and scaffold an empty `NuiBackend.{kt,swift}` once. They do **not** emit any base-URL
constant, networking config, ATS exception, or cleartext allowance. Your API base URL and all
networking live in app-owned `*BackendConnector.{kt,swift}` classes or shared backend config, with
`NuiBackend.{kt,swift}` kept as thin delegators, plus the platform network config you add. So "flipping
the base URL" = editing a constant **you** introduced in connector/shared config.

### 1. Define ONE base-URL constant in connector/shared config, env-switched
```kotlin
...
### 3. Plaintext HTTP in dev needs an explicit opt-in on BOTH platforms
The exporter emits no HTTP allowance, and modern iOS/Android block cleartext by default. For a
