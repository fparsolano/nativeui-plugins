...
fidelity. Both live entirely in app-owned backend files: thin `NuiBackend.kt` / `NuiBackend.swift`
delegators plus durable `*BackendConnector.kt` / `*BackendConnector.swift` classes. Nothing here touches
...
**Notes**
- `controls.tripTitle` is the **typed accessor** the generated `NuiScreenControls` exposes for a node whose
  `id` is `trip_title` (accessor = id camelCased). Renaming/deleting that node makes the accessor fail loudly
  at runtime by design — update the connector on both platforms when an id changes.
- Keep the two platforms **mirrored** — same `target` cases, same canned shapes — so the apps behave
  identically. (Standing rule: always do both platforms for one design.)
