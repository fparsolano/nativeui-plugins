...
allowed-tools: "Read Write Edit Glob Grep Bash(node ${CLAUDE_SKILL_DIR}/../../bin/*) Bash(node */nativeui-plugin/bin/*)"
---
...
## Three concept selection

Before creating the final styling guide or HTML, show **exactly three** user-selectable concept cards. Each card
must include a concept name, visual language, hierarchy/navigation pattern, responsive behavior, motion/interaction
character, and its best-fit primary journey. Keep required brand assets, copy, ids, and functional constraints in
every option; the concepts must differ in product direction rather than only color. Ask the user to select one or
blend named aspects. Do not turn all three into full screens unless the user explicitly requests comparative builds.
...
```bash
node ${CLAUDE_SKILL_DIR}/../../bin/nui-design-guide.mjs init -o nativeui-design-guide.md --prompt "<user prompt>"
```
...
```bash
node ${CLAUDE_SKILL_DIR}/../../bin/nui-design-guide.mjs check nativeui-design-guide.md
node ${CLAUDE_SKILL_DIR}/../../bin/nui-responsive-audit.mjs <screen.html...>
node ${CLAUDE_SKILL_DIR}/../../bin/nui-flow-audit.mjs <screen.html...>
```
