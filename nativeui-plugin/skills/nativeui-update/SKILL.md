...
`project.json`, reconcile it before changing files:
```bash
node <bin>/nui-project-sync.mjs sync project.json --name "Project Name" --human
```
This pulls cloud-only visual editor edits and pushes local-only edits. If local and cloud both changed, stop and
ask whether to pull the cloud copy, keep local as a new draft, or manually merge. Do not overwrite visual tweaks
the user may have made in the web or desktop editor. Run the same `sync` command again after the local update,
