...
## Default model-authored profile: canonical model + runtime parity first
NativeUI will keep expanding CSS coverage, but generated app HTML should stay inside the subset that the
canonical model, Rust editor/runtime, and webapp can render identically. The goal is `html == model == preview == webapp` first; native lanes
...
Avoid browser-only features in model output unless a parity fixture proves the Rust and web runtimes render them the
