...
```jsonc
{
  ...
  "stageWidth": 412.0,              // authored stage px width  (default 412)
  "stageHeight": 915.0,             // authored stage px height (default 915)
  ...
  "responsiveLayoutVersion": 1,     // 1 = nodes carry nui.semantic* twins; null/0 = legacy fixed
...
| `stageWidth` / `stageHeight` | `Double` | authored device px (default `412 × 915`). |
...
| `divisions` | `DivisionState[]` | smart-division breakpoints. Absent/empty = non-responsive. |
| `responsiveLayoutVersion` | `Integer` | `1` when responsive twins are present. Leave as imported. |
...
| `width` / `height` | `0` | resolved box size (px). Often mirrors `prefWidth/Height`. |
| `prefWidth` / `prefHeight` | `-1` (`USE_COMPUTED_SIZE`) | preferred size. `-1` = auto. Set both to pin a box. |
...
| Accessibility | `accessibleText`, `accessibleHelp`, `accessibleRole`, `accessibleRoleDescription` (`String`); `accessibilityExcluded` (`Boolean`). NOTE: `<img alt>` imports here; `aria-*`/`role`/`title` are NOT imported — set non-image a11y from `NuiBackend`. |
...
preview/export contract, not the live fetch itself: runtime API/database rows still belong in matching
Android + iOS connector classes. Native exports stamp the expanded preview rows with generated `NuiRepeaterBinding`
metadata and expose `controls.bindRepeater("<adapter-or-source-id>", rows)` for fixed row-pool binding. The
connector still fetches/maps the rows and calls that helper on both platforms. Never put secrets in
`dataSource`, adapters, or `sampleItems`.
...
**`nui.*` keys** (CSS semantics the importer baked). Commonly seen (not exhaustive):
- Clipping / overflow: `nui.clipToBounds`, `nui.cssClipBox`, `nui.cssClipPath`, `nui.cssTextOverflow`, `nui.svgDisplayBox`
- Text/line-box: `nui.cssLineHeightPx`, `nui.cssLetterSpacing`, `nui.cssWhiteSpace`, `nui.singleLineSelfSize`, `nui.noWrapNaturalText`, `nui.textTop`
- Flex: `nui.cssDisplayFlex`, `nui.cssFlexDirection`, `nui.cssJustifyContent`, `nui.flexGrowWeight`, `nui.flexSpacer`, `nui.hboxFillHeight`, `nui.vboxFillWidth`
- Grid: `nui.gridTemplateColumns`, `nui.gridAutoRowHeight`, `nui.gridAbsoluteOnlyItem`, `nui.columnGap`
- Gaps: `nui.hgap`, `nui.vgap`
- Responsive twins: `nui.semantic`, `nui.semanticWidth`, `nui.semanticHeight`, `nui.semanticFlexGrow`, `nui.semanticFlexReserved`
- Positioning: `nui.cssPosition`, `nui.cssPositionFixed`, `nui.pctTop|pctBottom|pctLeft|pctRight`, `nui.unmanaged`, `nui.fixedBodyWidth`, `nui.inferredFlowWidth`
- Sizing flags: `nui.authoredWidth|authoredHeight`, `nui.cssWidthExplicit`, `nui.cssHeightPx`
- CSS borders: `nui.cssBorder|cssBorderTop|cssBorderBottom|cssBorderLeft|cssBorderRight`, `nui.borderStyle`, `nui.strokeDashArray`

> These encode **importer decisions**. Hand-editing a `nui.*` value is advanced and easy to get subtly wrong —
> if you need a layout/CSS-semantics change, **re-author the HTML and re-import**. Bare anchor/margin values
> (e.g. `anchor.top`) are safe to nudge for absolute positioning.
...
- moving/resizing an **absolutely positioned** node (`layoutX/Y` on an AnchorPane/Pane child, or
  `anchor.top/left/...`; `prefWidth/prefHeight`);
