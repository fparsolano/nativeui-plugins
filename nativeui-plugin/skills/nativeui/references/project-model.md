...
importer/exporter and atomically splice it into an existing `project.json`:

- **`nui-fragment-import.mjs`** — HTML/CSS snippet (one `<div class="card">…</div>` etc.) → an atomic
  project update selected by `--replace <id>` or `--append-to <id>`. `--recipe` adds metadata-only patches,
  every replacement applies a complete no-op-baselined authored delta so unprojected canonical content survives,
  `--update-shared-library` directs that delta to a Library definition, and
  `--dry-run` validates the whole operation without writing.
...
Always dry-run, apply, then repeat the identical dry-run and require a no-op. For shared Library edits, verify
that the occurrence shell's geometry and unrelated shared defaults are unchanged. The splice reconciles stable
ids even when a no-op importer projection introduces layout-only wrappers, and fails atomically on ambiguity,
duplicate ids, dependency drift, or an incomplete shared baseline. Never manually replace `rootNodes` or merge
fragment `libraryItems`. See SKILL.md Iterate → "Granular fragment round-trip" for the exact commands.
