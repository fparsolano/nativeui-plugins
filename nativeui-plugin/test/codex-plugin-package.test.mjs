...
test('Codex bundle generator creates marketplace, plugin manifest, admin kit, and rewrites Claude placeholders', () => {
  ...
  assert.match(manifest.version, /^0\.3\.0\+codex\.[0-9]+$/);
  ...
  assert.ok(fs.existsSync(path.join(out, 'plugins/nativeui/lib/auth.mjs')));
  assert.ok(fs.existsSync(path.join(out, 'plugins/nativeui/mcp/server.mjs')));
  assert.ok(fs.existsSync(path.join(out, 'plugins/nativeui/admin/codex-mcp.example.toml')));
