...
test('sync pulls a cloud-only editor change and refreshes the local sidecar', async () => {
  const { file } = projectFile();
  const original = fs.readFileSync(file, 'utf8');
  const cloudText = JSON.stringify({ version: 4, stages: [{ name: 'Edited in web app', rootNodes: [] }] }, null, 2);
  fs.writeFileSync(`${file}.nativeui-sync.json`, JSON.stringify({
    version: 2, projectId: 'p1', name: 'App', revision: 1, contentHash: hashText(original),
  }));
  const { server, url } = await serve((req, res) => {
    res.setHeader('Content-Type', 'application/json');
    if (req.method === 'GET' && req.url === '/projects') {
      res.end(JSON.stringify({ items: [{ id: 'p1', name: 'App', revision: 2 }] }));
    } else if (req.method === 'GET' && req.url === '/projects/p1/content') {
      res.end(JSON.stringify({ contentJson: cloudText, revision: 2 }));
    } else {
      res.statusCode = 404;
      res.end(JSON.stringify({ error: 'not found' }));
    }
  ...
  try {
    const r = await runBinAsync('nui-project-sync.mjs', ['sync', file, '--name', 'App'], { env: authedEnv(url) });
    assert.equal(r.status, 0, r.stderr);
    assert.equal(JSON.parse(r.stdout).action, 'pulled');
    assert.equal(fs.readFileSync(file, 'utf8'), cloudText);
    assert.equal(JSON.parse(fs.readFileSync(`${file}.nativeui-sync.json`, 'utf8')).revision, 2);
  } finally {
    server.close();
  }
});

test('sync pushes a local-only agent change with the recorded revision', async () => {
  const { file } = projectFile();
  const original = fs.readFileSync(file, 'utf8');
  const localText = JSON.stringify({ version: 4, stages: [{ name: 'Edited by agent', rootNodes: [] }] }, null, 2);
  fs.writeFileSync(file, localText);
  fs.writeFileSync(`${file}.nativeui-sync.json`, JSON.stringify({
    version: 2, projectId: 'p1', name: 'App', revision: 1, contentHash: hashText(original),
  }));
  let received = null;
  const { server, url } = await serve((req, res) => {
    res.setHeader('Content-Type', 'application/json');
    if (req.method === 'GET' && req.url === '/projects') {
      res.end(JSON.stringify({ items: [{ id: 'p1', name: 'App', revision: 1 }] }));
    } else if (req.method === 'GET' && req.url === '/projects/p1/content') {
      res.end(JSON.stringify({ contentJson: original, revision: 1 }));
    } else if (req.method === 'PUT' && req.url === '/projects/p1/content') {
      let body = '';
      req.on('data', (chunk) => { body += chunk; });
      req.on('end', () => {
        received = JSON.parse(body);
        res.end(JSON.stringify({ id: 'p1', name: 'App', revision: 2 }));
      });
    } else {
      res.statusCode = 404;
      res.end(JSON.stringify({ error: 'not found' }));
  ...
  try {
    ...
    assert.deepEqual(received, { contentJson: localText, expectedRevision: 1 });
  } finally {
    server.close();
  }
});
