...
function collectStageInteractions(stage) {
  const out = [];
  for (const spec of Array.isArray(stage.interactionSpecs) ? stage.interactionSpecs : []) {
    if (spec && typeof spec === 'object') {
      out.push({ ...normalizeInteraction(spec), scope: 'stage', nodeId: null });
    }
  }
  walkNodes(stage.rootNodes, (node) => {
    for (const spec of Array.isArray(node.interactionSpecs) ? node.interactionSpecs : []) {
      if (spec && typeof spec === 'object') {
        out.push({ ...normalizeInteraction(spec), scope: 'node', nodeId: node.id || null });
      }
    }
  });
  return out;
...
function normalizeInteraction(spec) {
  ...
  return {
    ...
    target: action.targetLibraryItemId || action.targetNodeId || action.targetStageId || '',
