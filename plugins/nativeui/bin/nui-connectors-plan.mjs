...
const USAGE = 'Usage: node bin/nui-connectors-plan.mjs [project.json] [--json|--human] [-o plan.json]';
...
async function main() {
  try {
    const { project: projectPath, format, out } = parseArgs(process.argv.slice(2));
    let text;
    try {
    ...
    const plan = buildPlan(project, projectPath);
    const payload = format === 'human' ? human(plan) : JSON.stringify(plan, null, 2);
    if (out) {
