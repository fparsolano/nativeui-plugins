...
//   sync   <project.json> (--project-id <id> | --name <name>) [--location <folder>] [--json|--human]
...
const USAGE = 'Usage: node bin/nui-project-sync.mjs status|sync|pull|push <project.json> (--project-id <id> | --name <name>) [--location <folder>] [--json|--human]';

function parseArgs(argv) {
  const command = argv.shift();
  if (!['status', 'sync', 'pull', 'push'].includes(command)) throw new SyncError(USAGE);
  let file = '';
  let projectId = '';
  let name = '';
  let location = '';
  ...
  if ((command === 'push' || command === 'sync') && !name) throw new SyncError(`${command} requires --name so a missing cloud project can be created.`);
...
async function writeMeta(file, meta) {
  const out = {
    version: 2,
    ...meta,
    baseContentHash: meta.baseContentHash || meta.contentHash || '',
...
async function main() {
  try {
    ...
    if (!cloudRow && !['push', 'sync'].includes(opts.command)) throw new SyncError('Cloud project not found.');
    ...
    if (opts.command === 'sync') {
      if (!cloudRow) {
        // No cloud copy yet: the normal push path below creates it and records the sidecar.
        opts.command = 'push';
      } else {
        const cloudContent = await fetchContent(base, token, cloudRow.id);
        const report = summarizeStatus({ localText, meta, cloudRow, cloudContent });
        if (report.conflict) {
          const err = new SyncError('Local and cloud copies both changed since the last sync. Resolve the conflict explicitly; neither copy was overwritten.');
          err.code = 'revision_mismatch';
          throw err;
        }
        if (report.inSync) {
          emit({ action: 'in-sync', projectId: cloudRow.id, name: cloudRow.name, revision: cloudContent.revision, metadata: sidecarPath(opts.file) }, opts.format);
          return;
        }
        if (!meta) {
          if (report.localHash === report.cloudHash) {
            const written = await writeMeta(opts.file, {
              projectId: cloudRow.id, name: cloudRow.name, location: cloudRow.location || '', revision: cloudContent.revision,
              updatedAt: cloudContent.updatedAt || cloudRow.updated_at || '', contentHash: report.localHash, lastDirection: 'adopted',
            });
            emit({ action: 'adopted', projectId: cloudRow.id, name: cloudRow.name, revision: written.revision, metadata: sidecarPath(opts.file) }, opts.format);
            return;
          }
          const err = new SyncError(`Cloud project '${cloudRow.name || cloudRow.id}' exists but this local file has no matching sync base. Pull it, save local work under a new name, or merge explicitly.`);
          err.code = 'revision_mismatch';
          throw err;
        }
        if (report.cloudChanged && !report.localChanged) {
          await fs.writeFile(path.resolve(opts.file), cloudContent.contentJson);
          const written = await writeMeta(opts.file, {
            projectId: cloudRow.id, name: cloudRow.name, location: cloudRow.location || '', revision: cloudContent.revision,
            updatedAt: cloudContent.updatedAt || cloudRow.updated_at || '', contentHash: hashText(cloudContent.contentJson), lastDirection: 'pull',
          });
          emit({ action: 'pulled', projectId: cloudRow.id, name: cloudRow.name, revision: written.revision, metadata: sidecarPath(opts.file) }, opts.format);
          return;
        }
        if (report.localChanged && !report.cloudChanged) {
          // Continue into the existing revision-protected push path below.
          opts.command = 'push';
        } else {
          const err = new SyncError('Could not determine a safe sync direction. Check cloud status and resolve explicitly.');
          err.code = 'revision_mismatch';
          throw err;
        }
      }
    ...
    let action;
    if (cloudRow) {
      let expectedRevision = meta?.projectId === cloudRow.id ? meta.revision : undefined;
      if (expectedRevision == null) {
        const current = await fetchContent(base, token, cloudRow.id);
        if (hashText(current.contentJson || '') !== hashText(localText || '')) {
          const err = new SyncError(
            `Cloud project '${cloudRow.name || cloudRow.id}' already exists at revision ${current.revision}, but this local file has no matching sync base. Resume/pull it or save under a new name.`
          );
          err.code = 'revision_mismatch';
          throw err;
        }
