// nui-test-gen.mjs — generate iOS XCTest + Android JUnit/Robolectric tests for an
// EXPORTED NativeUI app that assert the NuiBackend CONTRACT.
//
...
//
// Usage:
//   node bin/nui-test-gen.mjs <project.json> --platform android|ios|both --out <exported-app-dir>
...
const USAGE =
  'Usage: node bin/nui-test-gen.mjs <project.json> --platform android|ios|both --out <dir> [--ios-out <dir>] [--package <pkg>]';
...
function parseArgs(argv) {
  ...
  if (!['android', 'ios', 'both'].includes(platform)) {
    throw new TestGenError(`--platform must be android|ios|both (got '${platform}').`);
  }
  if (!out) throw new TestGenError(`Missing --out <exported-app-dir>.\n${USAGE}`);
  return { project, platform, out, iosOut: iosOut || out, pkg };
}
...
async function main() {
  try {
    const { project, platform, out, iosOut, pkg } = parseArgs(process.argv.slice(2));
    ...
    if (platform === 'ios' || platform === 'both') {
      written.push(...(await genIos(parsed, named, path.resolve(iosOut))));
    }

    process.stdout.write(
      `Generated contract tests (named nodes: ${named.length}, platform: ${platform}):\n` +
