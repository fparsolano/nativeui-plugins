...
allowed-tools: "Read Write Edit Glob Grep Bash(node <bin>/*) Bash(node <bin>/*) Bash(git*) Bash(npm*) Bash(node*) Bash(python3*) Bash(docker*) Bash(gcloud*) Bash(fly*) Bash(flyctl*) Bash(railway*) Bash(render*) Bash(vercel*) Bash(netlify*)"
---
> Codex plugin path note: resolve `<bin>` as the NativeUI plugin's `bin/` directory and `<this-skill>` as `skills/nativeui-architect` inside the installed plugin source before running commands.
...
- **Client delivery:** exact NativeUI target IDs; for web, lane plus static/SSR build/hosting mode (never a
  reduction in responsive or dynamic behavior); for desktop, OS/CPU range and
  package route; for mobile, OS/store scope? Use `../nativeui/references/delivery-targets.md` for defaults.
...
- **Web delivery:** static asset host or Node runtime, routes/base path, frontend/API origins, CORS, cookie/session
  ownership, request-time personalization, offline fallback, and API/auth/user-response cache exclusions?
- **Distribution:** frontend provider/region/domain/TLS, mobile store/signing, desktop signing/notarization and
  update channel, observability/rollback, and which external actions are approved?
...
- selected delivery target IDs, web lane/render mode, supported OS/browser/CPU range, and responsive parent-
...
- client delivery/hosting: static versus Node host, routes/base path, domains/TLS, frontend/API origins, CORS,
  cookies/session ownership, PWA/offline/cache policy, packaging/signing, observability, and rollback;
...
- NativeUI wiring plan: registered library items, every selected target's manifest-declared durable seams,
  target-appropriate base URL/config switch, and equivalent behavior across the approved target set;
...
- use `nativeui-developer` / `nativeui-connect` to register API/database surfaces and wire every selected target;
...
- Keep architecture decisions aligned with every selected target: each API/database behavior must be callable
  through that target's manifest-declared durable seam, and web SSR/static build assumptions must match the
  host without dropping client interactions, state, timelines, offline behavior, or responsive constraints.
