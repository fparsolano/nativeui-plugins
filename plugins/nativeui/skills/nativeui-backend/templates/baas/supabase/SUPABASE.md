...
auto-generated **REST + realtime** API — no server to write or deploy. The Kotlin/Swift snippets below are
**flagship-mobile examples**; Rust, C#, and authored web targets must use their own manifest-declared durable
seams. For the flagship pair, the app talks to Supabase from app-owned `*BackendConnector.{kt,swift}` classes,
with `NuiBackend.{kt,swift}` kept as thin delegators. You write *zero* backend HTTP code.
...
## 6. Implement every selected target
Implement equivalent behavior for the same `target`s through every selected target's manifest-declared seam.
When the flagship mobile pair is selected, implement both Android and iOS connector files in each language's
idioms and keep `NuiBackend.kt` and `NuiBackend.swift` as thin delegators. Behavior only — anything visual stays
in the HTML/CSS design and is re-imported.
