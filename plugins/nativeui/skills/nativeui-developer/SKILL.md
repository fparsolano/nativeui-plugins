...
Before any hosted NativeUI operation, call `nativeui_auth_status` when MCP is available. If it reports `authenticated: false`, call `nativeui_login_start`, give the user its exact browser URL and code, then call `nativeui_login_wait` after they approve. Protected MCP tools repeat this verification themselves; do not begin hosted work while it is incomplete. For CLI-only use, run `preflight.mjs`, then `login.mjs` if prompted.
...
3. Use the typed asynchronous `NuiActionResult` contract for new logic. Generated adapters keep the stable
   synchronous hooks source-compatible. Results may carry typed control mutations, list data, navigation, and
   structured errors.
...
7. Use guarded editor/project synchronization: for an opted-in named cloud project, run
   `nui-project-sync sync project.json --name "My App"` before and after every local change. It pulls cloud-only
   visual editor edits, pushes local-only changes, and stops on local/cloud conflicts; publish only against the
...
3. For cloud/editor work use `nui-editor handoff|resume|publish`. For one-screen changes use
   `nui-screen-extract` and `nui-screen-update`; preserve other screens and metadata. For one component or
   section, use `nui-fragment-extract`, then project-mode `nui-fragment-import --project ... --replace ...`
   (or `--append-to`) with a dry-run before the write and an identical dry-run afterward. Use
   the replacement's automatic no-op baseline to preserve canonical content absent from HTML. Add
   `--update-shared-library` for a Library target and verify its thin occurrence shell and unrelated shared
   defaults remain unchanged; never hand-splice fragment JSON.
