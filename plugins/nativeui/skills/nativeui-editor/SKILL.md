...
allowed-tools: "Bash(node <bin>/*) Bash(node <bin>/*) Read Write Edit Glob"
---
> Codex plugin path note: resolve `<bin>` as the NativeUI plugin's `bin/` directory and `<this-skill>` as `skills/nativeui-editor` inside the installed plugin source before running commands.
...
For a named project that should stay reconciled after the editor session, prefer the one-step guarded loop:
```bash
node <bin>/nui-project-sync.mjs sync project.json --name "My App" --human
```
It pulls web-editor-only changes, pushes local-only changes, and stops when both changed. Run it before and after
each local round-trip; it is intentionally not a background watcher.
