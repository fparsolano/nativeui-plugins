...
---
> Codex plugin path note: resolve `<bin>` as the NativeUI plugin's `bin/` directory and `<this-skill>` as `skills/nativeui-import` inside the installed plugin source before running commands.
...
`<this-skill>/../nativeui/references/authoring-rules.md`); never edit around an import error.
