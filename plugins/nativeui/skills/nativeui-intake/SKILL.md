...
---
> Codex plugin path note: resolve `<bin>` as the NativeUI plugin's `bin/` directory and `<this-skill>` as `skills/nativeui-intake` inside the installed plugin source before running commands.
...
- requested delivery surface, exact target family/lane, web render mode, hosting/distribution, OS/browser range,
  and any unanswered decision from `../nativeui/references/delivery-targets.md`;
- screens/pages and navigation;
- reusable components/chrome;
- explicit support bounds and source-authored breakpoints when present; otherwise derive validation snapshots
  from the selected targets and content without inventing a preferred authoring canvas;
- parent ownership, fill/grow/shrink, min/max, scroll owner, and anchors for each major region;
...
If the source is not production-ready HTML, or the HTML is plain, interaction-free, or non-responsive, hand the bundle to
...
The audit must pass for generated app flows. Responsive behavior is the default and the deprecated
`--allow-static` audit opt-out does not satisfy the workflow; its name is unrelated to static web build/hosting
mode. Fix failures by adding real responsive structure: `@media` width breakpoints, `%`, `fr`,
...
```
Then implement the plan in every selected manifest-declared durable seam. Keep generated UI read-only.
