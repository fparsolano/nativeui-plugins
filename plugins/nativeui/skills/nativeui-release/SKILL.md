...
allowed-tools: "Bash(node <bin>/*) Bash(node <bin>/*) Bash(dotnet*) Bash(cargo*) Bash(gradle*) Bash(./gradlew*) Bash(xcodebuild*) Bash(xcrun*) Bash(fastlane*) Bash(vercel*) Bash(netlify*) Read Glob"
---
> Codex plugin path note: resolve `<bin>` as the NativeUI plugin's `bin/` directory and `<this-skill>` as `skills/nativeui-release` inside the installed plugin source before running commands.
