...
```bash
node <bin>/nui-project-sync.mjs sync project.json --name "My App" [--location "Folder"]
```
...
node <bin>/nui-export.mjs project.json --target web-react -o ./native-out
node <bin>/nui-export.mjs project.json --target desktop -o ./native-out
```
...
# edit card.html ...
node <bin>/nui-fragment-import.mjs card.html \
...
`node <bin>/nui-run.mjs project.json --target auto` (skill: **nativeui-run**).
