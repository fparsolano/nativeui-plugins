...
`onCallDatabase(target, params)`, `onPlayTimeline(target, params)`, `onOpenUrl(url, params)`,
`onSubmitForm(target, params)`, `onSetState(target, params)`, and `onRunScript(handler, params)`.

## What you write (UIKit/Android Views example)
```kotlin
...
- **One seam, no twin.** Everything goes in the single write-once `src/app_actions.rs` — a `struct AppActions`
  ...
  `app_actions.rs.new`.
- **Stable default hooks.** `on_screen_ready`, `on_navigate_to_stage`, `on_call_api`,
  `on_call_database`, `on_play_timeline`, `on_open_url`, `on_submit_form`, `on_set_state`, `on_run_script`,
  `on_animate_panel`, `fetch_list`, and `fetch_value` all have defaults. Rust and mobile route the same authored
  external-action families through real target hooks.
...
- **Async result path plus stable adapters.** New handlers return a future resolving to `NuiActionResult`;
  synchronous trait hooks remain source-compatible through the default adapter.
